...
const regions = [
  ...
  { name: "EU (Stockholm)",            key: "eu-north-1",     endpoint: "s3-website.eu-north-1.amazonaws.com" },
...
const credentialsFromProfile = (profileKey) => {
  ...
  if (!credentials.accessKeyId) {
    throw new KnownError(
      `An AWS profile named ${profileKey} does not exist in ${credentialsFilePath}`,
    )
  }
...
const credentialsFromEnvironment = () => {
  ...
  if (!credentials.accessKeyId) {
    throw new KnownError(
      "Environment variables AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY are not set",
    )
  }
